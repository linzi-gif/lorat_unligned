...
from trackit.datasets.UAMMSOT.specialization.memory_mapped.dataset import UnalignedMultiModalObjectTrackingDatasetFrame_MemoryMapped
from trackit.datasets.SOT.specialization.memory_mapped.dataset import SingleObjectTrackingDatasetSequence_MemoryMapped_MM, SingleObjectTrackingDatasetFrame_MemoryMapped_MM
...
#         L = min(v.shape[0], i.shape[0])
#         v, i = v[:L], i[:L]
#     else:
#         L = min(v.shape[0])
#         v, i = v[:L], i[:L]
#     return np.logical_and(v, i)


class UAMMSOTFrame(TrackingDataset_FrameInTrack, TrackingDataset_Frame):
    def __init__(self, frame: SingleObjectTrackingDatasetFrame_MemoryMapped_MM, category_id: Optional[int]):
        self._frame = frame
    ...
    def get_frame_size_v(self) -> np.ndarray:
        return self._frame.get_image_size_v()
    
    def get_frame_size_i(self) -> np.ndarray:
        return self._frame.get_image_size_i()
    ...
    def get_bounding_box_v(self) -> np.ndarray:
        if not self._frame.has_bounding_box():
            return np.full((4, ), float('nan'), dtype=np.float64)
        return self._frame.get_bounding_box_v().astype(np.float64)
    
    def get_bounding_box_i(self) -> np.ndarray:
        if not self._frame.has_bounding_box():
            return np.full((4, ), float('nan'), dtype=np.float64)
        return self._frame.get_bounding_box_i().astype(np.float64)
    
    def get_existence_flag_v(self) -> bool:
        if not self._frame.has_bounding_box():
            return False
        return self._frame.get_bounding_box_validity_flag_v().item() if self._frame.has_bounding_box_validity_flag() else True
    
    def get_existence_flag_i(self) -> bool:
        if not self._frame.has_bounding_box():
            return False
        return self._frame.get_bounding_box_validity_flag_i().item() if self._frame.has_bounding_box_validity_flag() else True
...
class UAMMSOTTrack(TrackingDataset_Track):
    def __init__(self, sequence: SingleObjectTrackingDatasetSequence_MemoryMapped_MM, category_id: Optional[int]):
    ...
    def __getitem__(self, index: int) -> TrackingDataset_FrameInTrack:
        return UAMMSOTFrame(self._sequence[index], self._category_id)

    def get_all_frame_size_v(self) -> np.ndarray:
        return self._sequence.get_all_frame_sizes_v()
    
    def get_all_frame_size_i(self) -> np.ndarray:
        return self._sequence.get_all_frame_sizes_i()
    
    def get_all_object_bounding_box_v(self) -> np.ndarray:
        if not self._sequence.has_bounding_box():
            return np.full((len(self), 4), float('nan'), np.float64)
        return self._sequence.get_all_bounding_boxes_v()
    
    def get_all_object_bounding_box_i(self) -> np.ndarray:
        if not self._sequence.has_bounding_box():
            return np.full((len(self), 4), float('nan'), np.float64)
        return self._sequence.get_all_bounding_boxes_i()
    
    def get_all_object_existence_flag(self) -> Optional[np.ndarray]:
        if not self._sequence.has_bounding_box():
            return np.zeros((len(self), ), dtype=bool)
        return self._sequence.get_all_bounding_box_validity_flags_v() if self._sequence.has_bounding_box_validity_flag() else None
...
class UAMMSOTSequence(TrackingDataset_Sequence):
    def __init__(self, sequence: SingleObjectTrackingDatasetSequence_MemoryMapped_MM):
        self._sequence = sequence
    ...
    def get_track_by_index(self, index: int) -> TrackingDataset_Track:
        assert index == 0
        return UAMMSOTTrack(self._sequence, self._category_id)
...
class UAMMSOTDataset(TrackingDataset):
    def __init__(self, dataset: UnalignedMultiModalObjectTrackingDatasetFrame_MemoryMapped):
    ...
    def __getitem__(self, index: int) -> TrackingDataset_Sequence:
        return UAMMSOTSequence(self._dataset[index])
