...
from trackit.datasets.SOT.specialization.memory_mapped.dataset import SingleObjectTrackingDatasetSequence_MemoryMapped, SingleObjectTrackingDatasetFrame_MemoryMapped
from trackit.data.source import *
from trackit.miscellanies.image.io import *


class MMOTFrame(TrackingDataset_FrameInTrack, TrackingDataset_Frame):
...
class MMOTTrack(TrackingDataset_Track):
    ...
    def __getitem__(self, index: int) -> TrackingDataset_FrameInTrack:
        return MMOTFrame(self._sequence[index], self._category_id)
...
class MMOTSequence(TrackingDataset_Sequence):
    ...
    def get_track_by_index(self, index: int) -> TrackingDataset_Track:
        assert index == 0
        return MMOTTrack(self._sequence, self._category_id)
...
class MMOTDataset(TrackingDataset):
    def __init__(self, dataset: MultiModalObjectTrackingDataset_MemoryMapped):
    ...
    def __getitem__(self, index: int) -> TrackingDataset_Sequence:
        return MMOTSequence(self._dataset[index])
