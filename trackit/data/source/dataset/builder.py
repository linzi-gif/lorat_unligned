...
from trackit.datasets.UAMMSOT.specialization.memory_mapped.dataset import UnalignedMultiModalObjectTrackingDataset_MemoryMapped
...
from .MMOT import MMOTDataset
from .UAMMSOT import UAMMSOTDataset
...
def build_datasets_from_config(config: dict):
    ...
    for dataset_building_parameter in config['datasets']:
        ...
        elif dataset_type == 'MMOT':
            from trackit.datasets.MMOT.factory import MultiModalObjectTrackingDatasetFactory
            module = importlib.import_module('trackit.datasets.MMOT.datasets.{}'.format(dataset_name))
            factory_class = MultiModalObjectTrackingDatasetFactory
        elif dataset_type == 'UAMMSOT':
            from trackit.datasets.UAMMSOT.factory import UnalignedMultiModalObjectTrackingDatasetFactory
            module = importlib.import_module('trackit.datasets.UAMMSOT.datasets.{}'.format(dataset_name))
            factory_class = UnalignedMultiModalObjectTrackingDatasetFactory  
...
def build_dataset_as_data_source(data_source_config: dict):
    ...
    for dataset in datasets:
        ...
        elif isinstance(dataset, MultiModalObjectTrackingDataset_MemoryMapped):
            wrapped_datasets.append(MMOTDataset(dataset))
        elif isinstance(dataset, UnalignedMultiModalObjectTrackingDataset_MemoryMapped):
            wrapped_datasets.append(UAMMSOTDataset(dataset))
        else:
