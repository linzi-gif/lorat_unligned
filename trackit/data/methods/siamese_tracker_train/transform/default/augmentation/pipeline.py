...
class Augmentation_TwoBox:
    def __call__(self, images: Sequence[torch.Tensor], bboxes: Sequence[np.ndarray], bboxes_i: Sequence[np.ndarray], rng_engine: np.random.Generator) -> Tuple[Sequence[torch.Tensor], Sequence[np.ndarray]]:
        raise NotImplementedError
...
class AugmentationPipeline:
    ...
    def __call__(self, context: Dict[str, Sequence[UnalignedAnnotatedImage]], rng_engine: np.random.Generator):
        for augmentation in self.augmentations:
            ...
            augmentation_with_twobox = isinstance(augmentation_fn, Augmentation_TwoBox)
            ...
            elif augmentation_with_twobox:
                bboxes, bboxes_i = self._collect_bboxes(context, targets)
                images, bboxes, bboxes_i = self._apply_augmentation_with_twoboxes(images, bboxes, bboxes_i, augmentation, rng_engine)
            else:
                images = self._apply_image_only_augmentation(images, augmentation, rng_engine)

            self._update_context(context, targets, images, bboxes if augmentation_with_twobox else None, bboxes_i if augmentation_with_twobox else None)

    def _collect_images(self, context: Dict[str, Sequence[UnalignedAnnotatedImage]], targets: Sequence[str]
    ...
    def _collect_bboxes(self, context: Dict[str, Sequence[UnalignedAnnotatedImage]], targets: Sequence[str]
                        ) -> Sequence[np.ndarray]:
        return [annotated_image.bbox for target in targets for annotated_image in context[target]], [annotated_image.bbox_i for target in targets for annotated_image in context[target]]
    ...
    def _apply_augmentation_with_twoboxes(self, images: Sequence[torch.Tensor], bboxes: Sequence[np.ndarray], bboxes_i: Sequence[np.ndarray],
                                       augmentation: AugmentationConfig, rng_engine: np.random.Generator
                                       ) -> Tuple[Sequence[torch.Tensor], Sequence[np.ndarray]]:
        augmentation_fn = augmentation.augmentation_fn
        if augmentation.joint:
            return augmentation_fn(images, bboxes, bboxes_i, rng_engine)
        else:
            new_images, new_bboxes, new_bboxes_i = [], [], []
            for image, bbox, bbox_i in zip(images, bboxes, bboxes_i):
                augmented_images, augmented_bboxes, augmented_bboxes_i = augmentation_fn([image], [bbox], [bbox_i], rng_engine)
                new_images.append(augmented_images[0])
                new_bboxes.append(augmented_bboxes[0])
                new_bboxes_i.append(augmented_bboxes_i[0])
            return new_images, new_bboxes, new_bboxes_i
    ...
    def _update_context(self, context: Dict[str, Sequence[UnalignedAnnotatedImage]], targets: Sequence[str],
                        images: Sequence[torch.Tensor], bboxes: Sequence[np.ndarray] = None, bboxes_i: Sequence[np.ndarray] = None):
        index = 0
        for target in targets:
            for annotated_image in context[target]:
                ...
                if bboxes is not None:
                    annotated_image.bbox_i = bboxes_i[index]
                index += 1
