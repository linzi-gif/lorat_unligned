...
from .pipeline import Augmentation, Augmentation_TwoBox
...
class HorizontalFlipAugmentation_TwoBox(Augmentation_TwoBox):
    def __init__(self, probability: float):
        self.probability = probability
    ...
    def __call__(self, images: Sequence[torch.Tensor], bboxes: Sequence[np.ndarray], bboxes_i: Sequence[np.ndarray], rng_engine: np.random.Generator) -> Tuple[Sequence[torch.Tensor], Sequence[np.ndarray]]:
        if self.probability > 0 and rng_engine.random() < self.probability:
            ...
            all_image_w = tuple(img.shape[-1] for img in images)
            bboxes = tuple(bbox_horizontal_flip(bbox, w) for bbox, w in zip(bboxes, all_image_w))
            bboxes_i = tuple(bbox_horizontal_flip(bbox, w) for bbox, w in zip(bboxes_i, all_image_w))
        return images, bboxes, bboxes_i