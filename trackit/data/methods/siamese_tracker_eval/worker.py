...
from ...source.dataset.MMOT import MMOTFrame
...
def _prepare_frame_context(frame: TrackingDataset_FrameInTrack):
    gt_bbox = frame.get_bounding_box_v() if frame.get_existence_flag_v() else None
    gt_bbox_i = frame.get_bounding_box_i() if frame.get_existence_flag_i() else None
    
    if isinstance(frame, UAMMSOTFrame):
        image_getter_fn = get_mmot_frame_decoder(frame)
    else:
        image_getter_fn = get_frame_decoder(frame)
    return SiameseTrackerEvalDataWorker_FrameContext(frame.get_frame_index(), image_getter_fn, gt_bbox, gt_bbox_i)
