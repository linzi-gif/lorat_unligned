...
def parse_optimizer_per_params_config(model: nn.Module, criterion: Optional[nn.Module], optimizer_config: dict):
    ...
    # 2025/12/21  模型可学习参数设置
    model_named_parameters = {name: param for name, param in model.named_parameters() if param.requires_grad and "lora" in name or "embed" in name or "head" in name or "fuse_search" in name}
