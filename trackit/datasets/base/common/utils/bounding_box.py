...
def get_bounding_box_v(object_: dict):
    bounding_box = object_['bounding_box_v']
    bounding_box_validity = None
    if 'validity' in bounding_box:
        bounding_box_validity = bounding_box['validity']
    return bounding_box['value'], bounding_box_validity

def get_bounding_box_i(object_: dict):
    bounding_box = object_['bounding_box_i']
    bounding_box_validity = None
    if 'validity' in bounding_box:
        bounding_box_validity = bounding_box['validity']
    return bounding_box['value'], bounding_box_validity
...
def set_bounding_box_v_(object_: dict, bounding_box: Union[Tuple[float, ...], np.ndarray], validity: bool = None, dtype=None, context=None):
    if isinstance(bounding_box, np.ndarray):
        bounding_box = bounding_box.tolist()
    if validity is not None:
        assert isinstance(validity, bool)
    ...
    if validity is not None:
        bounding_box['validity'] = validity
    object_['bounding_box_v'] = bounding_box
    if context is not None:
        context.set_bounding_box_dtype(dtype)


def set_bounding_box_i_(object_: dict, bounding_box: Union[Tuple[float, ...], np.ndarray], validity: bool = None, dtype=None, context=None):
    if isinstance(bounding_box, np.ndarray):
        bounding_box = bounding_box.tolist()
    if validity is not None:
        assert isinstance(validity, bool)
    ...
    if validity is not None:
        bounding_box['validity'] = validity
    object_['bounding_box_i'] = bounding_box
    if context is not None:
        context.set_bounding_box_dtype(dtype)
