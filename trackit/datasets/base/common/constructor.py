...
'name', 'split', 'version', 'filters', 'type', 'category_id_name_map', 'images', 'context')
...
'name', 'split', 'version', 'filters', 'type', 'category_id_name_map', 'sequences', 'context')
...
video_dataset_frame_object_key_exclude_list = ('id', 'bounding_box', 'bounding_box_v', 'bounding_box_i')
        ...
        if 'context' in dataset:
            ...
            if 'bounding_box_data_type' in dataset_context:
                if dataset_context['bounding_box_data_type'] == 'int':
                    ...
                    raise RuntimeError(
                        f"unknown value {dataset_context['bounding_box_data_type']} in dataset['bounding_box_data_type']")
            ...
            if 'bounding_box_coordinate_system' in dataset_context:
                self.bounding_box_coordinate_system = BoundingBoxCoordinateSystem[
                    dataset_context['bounding_box_coordinate_system']]
...
def set_mmot_path_(image_dict: dict, image_path: tuple, root_path: str, size):
    image_dict['path'] = [_add_path_impl(image_path[0], root_path), _add_path_impl(image_path[1], root_path)]
    if size is None:
        try:
            image_v = Image.open(image_path[0])
            image_i = Image.open(image_path[1])
            image_dict['size'] = [image_v.size, image_i.size]
        except UnidentifiedImageError:
            print(f'Warning: failed to decode image file {image_path}')
            image_dict['size'] = [(0, 0), (0, 0)]
    else:
        assert len(size) == 2
        for modailty in size:
            for v in modailty:
                if isinstance(v, float):
                    assert v.is_integer()
...
def generate_mmot_sequence_path_(sequence: dict):
    if 'frames' not in sequence:
        return
    paths = []
    for frame in sequence['frames']:
    ...
    paths_v = [sublist[0] for sublist in paths]
    paths_i = [sublist[1] for sublist in paths]
    sequence_path_v = os.path.commonpath(paths_v)
    sequence_path_i = os.path.commonpath(paths_i)
    sequence_path = [sequence_path_v, sequence_path_i]

    sequence['path'] = sequence_path
    for frame in sequence['frames']:
        frame['path'][0] = os.path.relpath(frame['path'][0], sequence_path[0])
        frame['path'][1] = os.path.relpath(frame['path'][1], sequence_path[1])
...
class BaseDatasetImageConstructor:
    def __init__(self, image: dict, root_path: str, context: _DatasetConstructionContext,
                 category_id_name_map: dict = None):
        self.image = image
        self.root_path = root_path
        self.context = context
...
class _BaseDatasetConstructor:
    def __init__(self, dataset: dict, root_path: str, dataset_type: str, schema_version: int, data_version: int,
                 context: _DatasetConstructionContext):
        if 'type' in dataset:
    ...
    def set_bounding_box_coordinate_system(self,
                                           bounding_box_coordinate_system: Union[BoundingBoxCoordinateSystem, str]):
        if isinstance(bounding_box_coordinate_system, str):
            bounding_box_coordinate_system = BoundingBoxCoordinateSystem[
                bounding_box_coordinate_system]  # convert string to enum
...
class BaseVideoDatasetConstructor(_BaseDatasetConstructor):
    def __init__(self, dataset: dict, root_path: str, data_version: int, context: _DatasetConstructionContext):
        super(BaseVideoDatasetConstructor, self).__init__(dataset, root_path, 'video', video_dataset.__version__,
                                                          data_version, context)
...
class BaseImageDatasetConstructor(_BaseDatasetConstructor):
    def __init__(self, dataset: dict, root_path: str, data_version: int, context: _DatasetConstructionContext):
        super(BaseImageDatasetConstructor, self).__init__(dataset, root_path, 'image', image_dataset.__version__,
                                                          data_version, context)
