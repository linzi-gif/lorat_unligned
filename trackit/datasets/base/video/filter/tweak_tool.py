...
from trackit.datasets.base.common.utils.manipulator import fit_objects_bounding_box_in_image_size_unaligned, \
    update_objects_bounding_box_validity_unaligned
...
class UAMMVideoDatasetTweakTool:
    def __init__(self, dataset: dict):
        self.manipulator = VideoDatasetManipulator(dataset)
    ...
    def bounding_box_fit_in_image_size(self, exclude_non_validity=True):
        for sequence in self.manipulator:
            for frame in sequence:
                fit_objects_bounding_box_in_image_size_unaligned(frame, self.manipulator.context_dao, exclude_non_validity)

    def bounding_box_update_validity(self, skip_if_mark_non_validity=True):
        for sequence in self.manipulator:
            for frame in sequence:
                update_objects_bounding_box_validity_unaligned(frame, self.manipulator.context_dao, skip_if_mark_non_validity)
    ...
    def annotation_standard_conversion(self, bounding_box_format: BoundingBoxFormat = None,
        ...
        for sequence in self.manipulator:
            for frame in sequence:
                for object_ in frame:
                    if object_.has_bounding_box():
                        bounding_box_v, bounding_box_validity_v = object_.get_bounding_box_v()
                        bounding_box_i, bounding_box_validity_i = object_.get_bounding_box_i()
                        bounding_box_v = converter(bounding_box_v)
                        bounding_box_i = converter(bounding_box_i)
                        object_.set_bounding_box(bounding_box_v, bounding_box_i, bounding_box_validity_v and bounding_box_validity_i)
    ...
    def bounding_box_remove_non_validity_objects(self):
        for sequence in self.manipulator:
            for frame in sequence:
                for object_ in frame:
                    if object_.has_bounding_box():
                        _, validity_v = object_.get_bounding_box_v()
                        _, validity_i = object_.get_bounding_box_i()
                        if validity_v == False or validity_i == False:
                            object_.delete()
    ...
    def remove_invalid_image(self):
        for sequence in self.manipulator:
            for frame in sequence:
                w_v, h_v = frame.get_image_size_v()
                w_i, h_i = frame.get_image_size_i()
                if w_v == 0 or h_v == 0 or w_i == 0 or h_i == 0:
                    frame.delete()
