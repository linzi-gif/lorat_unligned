...
class RGBT210_Seed(BaseSeed):
    def __init__(self, root_path: str = None):
        if root_path is None:
            # get the path from `consts.yaml` file
            root_path = self.get_path_from_config('RGBT210_PATH')

        super(RGBT210_Seed, self).__init__(
            'RGBT210',  # dataset name
    ...
    def construct(self, constructor: MultiModalObjectTrackingDatasetConstructor):
        ...
        with open('{}list_210.txt'.format(self.root_path)) as f:
