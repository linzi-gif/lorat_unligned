...
class GTOT_Seed(BaseSeed):
    def __init__(self, root_path: str = None):
        if root_path is None:
            # get the path from `consts.yaml` file
            root_path = self.get_path_from_config('GTOT_PATH')

        super(GTOT_Seed, self).__init__(
            'GTOT',  # dataset name
    ...
    def construct(self, constructor: MultiModalObjectTrackingDatasetConstructor):
        ...
        with open('{}gtot.txt'.format(self.root_path)) as f:
        ...
        for sequence_name in sequence_names:
            with constructor.new_sequence() as sequence_constructor:
                ...
                boxes_path = os.path.join(sequence_path, 'init1_i.txt')
                frames_path_v = os.path.join(sequence_path, 'v')
                frames_path_i = os.path.join(sequence_path, 'i')
                ...
                for frame_id_v, frame_id_i, box in zip(frame_ids_v, frame_ids_i, boxes):
                    ...
                    with sequence_constructor.new_frame() as frame_constructor:
                        ...
                        frame_constructor.set_path((frame_path_v, frame_path_i), image_size=None)
