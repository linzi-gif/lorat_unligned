...
class MMLoRAT_DINOv2(nn.Module):
    ...
    def forward(self, z: torch.Tensor, x: torch.Tensor, d: torch.Tensor,
        ...
        x_feat_v = self._fusion(z_feat_v, x_feat_v, d_feat_v)
        x_feat_i = self._fusion(z_feat_i, x_feat_i, d_feat_i)
    ...
    def _fusion(self, z_feat: torch.Tensor, x_feat: torch.Tensor, d_feat: torch.Tensor):
        fusion_feat = torch.cat((z_feat, x_feat, d_feat), dim=1)
        for i in range(len(self.blocks)):
            fusion_feat = self.blocks[i](fusion_feat)
        fusion_feat = self.norm(fusion_feat)
        return self._split_search(fusion_feat, z_feat.shape[1], x_feat.shape[1])
